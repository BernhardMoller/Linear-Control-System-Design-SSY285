 ...
 *
 * Date:           Thu Nov 24 11:15:38 2016
 * Model version:  1.599
 * Matlab version: 8.8 (R2015a) 09-Feb-2015
 ****************************************************************************/
...
typedef RT_MODEL_Experiment_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Experiment(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Experiment_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Experiment_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Experiment:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Experiment -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Experiment_DW.HILReadEncoderTimebase_Task,
        (t_clock) Experiment_P.HILReadEncoderTimebase_clock, 1000.0, -1)
        ;
      if (result == 0) {
